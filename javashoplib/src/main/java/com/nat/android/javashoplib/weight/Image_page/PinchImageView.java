...
public class PinchImageView extends ImageView {
    ...
    private boolean isSinglePonit=false;
	// 外界点击事件
	...
	public void setScrollLisener(ScrollListener lisener){
	...
	public boolean onTouchEvent(MotionEvent event) {
        ...
        if (action==MotionEvent.ACTION_UP&&event.getPointerCount()==1&&isSinglePonit){
            if (event.getPointerCount()==1&&newY - oldY > 0
                    && (Math.abs(newY - oldY) > 25)&&listener!=null) {
                //向下滑動
                listener.down();
            } else if (event.getPointerCount()==1&&newY - oldY < 0
                    && (Math.abs(newY - oldY) > 25)&&listener!=null) {
                //向上滑动
                listener.up();
            }
